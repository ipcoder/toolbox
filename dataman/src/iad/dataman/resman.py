...
class ResourceModel(YamlModel, extra=Extra.forbid):
    """
...
class ResManager(Generic[RT]):
    ...
    * Find a configuration by name.
    ...
    def _set_loc(self, folders, *, cache=False, auto_scan: str | AutoScan = 'first', **search_kws):
        ...
        :param search_kws: optional scan/guide keyword arguments for upstream tooling

        Typical keyword arguments include:
        ::
        ...
        if not (pattern := search_kws.pop("pattern", None)):
            ...
            pattern = make_yml_model_format('_', patterns=pattern, model=self.model).match_pattern
    ...
    def find_resource(self, name: str, *, valid=True, fail=False,
        ...
        if not info and fuzzy is not False and self._resreg:
            from iad.core.strings import fuzzy_find
    ...
    def __repr__(self):
        from iad.core.strings import short_form
    ...
    * Listing all the registered ConfigManagers
    ...
    def set_env_loc(cls, env: str | Path | bool = None):
        """Sets global environment locators.
         Convenient for some case, over generally preferable `algutils.env.EnvLoc.reset()`.

          - ``None``: sets to algutils.env defaults ONLY if is currently unset
          - ``True``: sets to the algutils.env defaults
...
def locatable(
    ...
    **Example:**
    ...
    loc_par = locals().copy()
    from .datacast.scan import GuideScan
    from iad.core.datatools import rm_keys
