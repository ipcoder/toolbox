...
from iad.core.regexp import is_regex
from iad.core.wrap import name_outputs
...
class MetaFormat(type):
    ...
    def match_pattern(cls) -> str:
        """Regex pattern matching *only the ending* part of the path.

        For example, if the format is ``*.txt``, the regexp matches a basename
        like ``name.txt`` (see also ``find_handler`` name_ext extraction).
        """
