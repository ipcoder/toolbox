...
from toolbox.io.format import FileFormat, CT, FormatHandler, MetaFormat
from toolbox.utils import logger, as_list, as_iter
...
def model_arguments(*, exclude: Union[str, List[str]] = None, config=None,
    ...
    """
    from toolbox.utils.datatools import issubset_report
    config = config or {}
    if exclude:
        config['extra'] = 'allow'
...
def _template_missing_defaults(model: type[pyd.BaseModel], _rep_models=None):
    from toolbox.utils.codetools import NamedObj
...
class YamlModel(YamlModelMixin, pyd.BaseModel):
    ...
    def _str_form(self, width=80, levels=3, indent=3 * ' ', exclude=None):
        # -------------
        from toolbox.utils.strings import repr_nested
