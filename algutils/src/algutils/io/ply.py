##############################################################################################
"""
This file contains functions to convert a 3D model to a .ply file
1. ply functions from https://github.com/daavoo/pyntcloud/blob/master/pyntcloud/io/ply.py
2. ply_save()      - save a depth map into a .ply file.
3. disp_to_ply()   - convert disparity to depth and save as .ply file in a single call
"""
##############################################################################################


#######################################################
# ply functions from https://github.com/daavoo/pyntcloud/blob/master/pyntcloud/io/ply.py
# modified by Ady to add color vertices
#######################################################
...
def describe_element(name, df):
    ...
    if name == 'face':
        ...
        # code below was modified to support vertex colors,
        # arriving as object type in a mixed-typed numpy array
        # original:
        ...
        if len(df.columns) != 6:
            ...
            element.append('property float ' + df.columns.values[0])
            element.append('property float ' + df.columns.values[1])
            element.append('property float ' + df.columns.values[2])
            element.append('property uchar ' + df.columns.values[3])
            element.append('property uchar ' + df.columns.values[4])
            element.append('property uchar ' + df.columns.values[5])
